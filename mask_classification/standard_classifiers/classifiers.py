...
class DataManager():
    ...
    def __init__(self, train_path, test_path):
        ...
        if self.train_classes != self.test_classes:
            printI("Error: expected training and test sets to contain the same set of labels:")
    ...
    def accuracy(self):
        """
        Display the accuracy for the current results.
        """
        if self.has_results == False:
            raise Exception("No results are available, cannot generate confusion matrix.")
        # print(f"Accuracy = {accuracy_score(self.test_labels, self.y_pred)}")
        N = 18
    ...
    def bert_embedding(self):
        ...
        for text in self.train_data:
            self.X_train.append(model.encode(text))
        for text in self.test_data:
            self.X_test.append(model.encode(text))
